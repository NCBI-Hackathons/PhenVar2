...
def create_tables(conn):
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS snps_publications (rsid INTEGER, publication_id INTEGER, FOREIGN KEY (publication_id) REFERENCES publications (id))''')
    conn.commit()
    return()

def add_snp(conn, rsid, publication_id):
    c = conn.cursor()
    c.execute("INSERT INTO snps_publications VALUES (?, ?)", (rsid, publication_id))
    conn.commit()
    return()
